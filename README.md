...
- [pylint](https://pylint.readthedocs.io/en/stable/): Ised to run simple code quality checks. This can be executed using `just lint` (to run all the linters) or `just pylint` (to just run pylint).
...
This project uses [FastAPI](https://fastapi.tiangolo.com) as its webserver. The service can be launch either hrough the default just target (simply run `just`) or through `just run`.
